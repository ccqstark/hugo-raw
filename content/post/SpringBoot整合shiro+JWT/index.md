---
title: "[SpringBoot]整合shiro+JWT做鉴权"
date: 2020-10-17T16:52:00+08:00
draft: true
slug: "springboot_shiro_jwt"
tags:
    - SpringBoot
categories:
    - JavaEE
...
public class JwtUtil {
    ...
    private static final String defaultbase64EncodedSecretKey = "wdnmd";
    ...
    public String encode(String iss, long ttlMillis, Map<String, Object> claims) {
        ...
        JwtBuilder builder = Jwts.builder()
                ...
                .setIssuedAt(new Date(nowMillis))// 这个地方就是以毫秒为单位，换算当前系统时间生成的iat
                .setIssuer(iss)// 签发人，也就是JWT是给谁的（逻辑上一般都是username或者userId）
                .signWith(signatureAlgorithm, base64EncodedSecretKey);//这个地方是生成jwt使用的算法和秘钥
        if (ttlMillis >= 0) {
            // 过期时间 = 当前时间 + 生存时间
            long expMillis = nowMillis + ttlMillis;
            Date exp = new Date(expMillis);// 过期时间，这个也是使用毫秒生成的
            builder.setExpiration(exp);
        }
        return builder.compact();
    }

    //相当于encode的逆向，传入jwtToken生成对应的username和password等字段。Claim就是一个map
    //也就是拿到荷载部分所有的键值对
    ...
    public static void main(String[] args) {
        JwtUtil util = new JwtUtil("wdnmd", SignatureAlgorithm.HS256);
        //以tom作为秘钥，以HS256加密
        Map<String, Object> map = new HashMap<>();
        map.put("username", "ccq");
        map.put("password", "1428");
        map.put("age", 20);

        String jwtToken = util.encode("ccqstark", 30000, map);
...
import com.ccqstark.springbootquick.util.JwtUtil;
...
import com.ccqstark.springbootquick.auth.*;
...
public class LoginController {
    ...
    public ResponseEntity<Map<String, String>> login(String username, String password) {
        ...
        JwtUtil jwtUtil = new JwtUtil("wdnmd", SignatureAlgorithm.HS256);
        Map<String, Object> chaim = new HashMap<>();
        chaim.put("username", username);

        // 生存时间在这里设置
        String jwtToken = jwtUtil.encode(username,  20*1000, chaim);
...
请求登录接口，密码正确后返回一个`token`，以后每次请求带上这个token以`header`里的`Authorization`字段的形式
